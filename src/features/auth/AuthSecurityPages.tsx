...
function AuthCard({
  ...
  return (
    ...
    >
      <Paper
        elevation={2}
        sx={{ width: '100%', maxWidth: 500, p: { xs: 3, sm: 4 }, borderRadius: 3 }}
      >
        <Box
          component="img"
          src="/brand/logo-yungas.jpeg"
          alt="Yungas Distribuidora"
          sx={{ width: 68, height: 68, objectFit: 'contain', mb: 2 }}
        />
...
export function RegisterPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const result = await registerRequest({
        ...
        requestedRole: form.requestedRole as 'VENDEDOR' | 'COBRADOR',
  ...
  return (
    <AuthCard
      ...
      subtitle="Primero verificarás tu correo y después el administrador revisará tu solicitud."
    >
      <Stack component="form" spacing={2} onSubmit={submit}>
        ...
        {registeredEmail && (
          ...
          >
            {resendSeconds > 0
              ? `Reenviar correo en ${resendSeconds}s`
...
export function VerifyEmailPage() {
  ...
  return (
    <AuthCard
      ...
      subtitle="Confirmamos que el correo te pertenece."
    >
      <Alert
        severity={
...
export function ForgotPasswordPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      setError(null);
      setMessage((await forgotPasswordRequest(email.trim())).message);
    } catch (value) {
      setError(errorMessage(value));
    }
  };
  return (
    <AuthCard
      ...
      subtitle="Enviaremos un enlace de recuperación si la cuenta existe."
    >
      <Stack component="form" spacing={2} onSubmit={submit}>
        {message && <Alert severity="success">{message}</Alert>}
        {error && <Alert severity="error">{error}</Alert>}
...
export function ResetPasswordPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      setError(null);
      setMessage((await resetPasswordRequest(token, password)).message);
    } catch (value) {
      setError(errorMessage(value));
    }
  };
  return (
    <AuthCard
      ...
      subtitle="La nueva clave cerrará las sesiones abiertas anteriormente."
    >
      <Stack component="form" spacing={2} onSubmit={submit}>
        {message && <Alert severity="success">{message}</Alert>}
        {error && <Alert severity="error">{error}</Alert>}
...
export function TwoFactorPage() {
  ...
  useEffect(() => {
    ...
    if (setupRequired) {
      startTwoFactorSetup(challengeToken)
        .then(setSetup)
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const result = useRecovery
        ? await useRecoveryCode(challengeToken, recoveryCode, remember)
        : setupRequired
          ? await confirmTwoFactor(challengeToken, code, remember)
          : await verifyTwoFactor(challengeToken, code, remember);
      completeLogin(result);
      sessionStorage.removeItem('auth_challenge');
      sessionStorage.removeItem('auth_setup');
  ...
  const copySecret = async () => {
    ...
    setMessage('Clave copiada. Agrégala en tu aplicación autenticadora.');
  };
  ...
  if (recoveryCodes) {
    return (
      <AuthCard
        ...
        subtitle="Cada código solo puede usarse una vez. Guárdalos fuera del sistema."
      >
  ...
  return (
    <AuthCard
      title={setupRequired ? 'Configurar segundo factor' : 'Verificación en dos pasos'}
      subtitle="Usa Google Authenticator, Microsoft Authenticator o una aplicación TOTP compatible."
    >
      <Stack component="form" spacing={2} onSubmit={submit}>
        {message && <Alert severity="success">{message}</Alert>}
        {error && <Alert severity="error">{error}</Alert>}
        {setup && (
          <Alert severity="info">
            <Stack spacing={1}>
              <Typography>
                En tu autenticador elige <strong>Ingresar clave de configuración</strong>.
              </Typography>
