...
export function DashboardPage() {
  ...
  const canSeeSales = user?.role !== 'COBRADOR';
  ...
  const debtAlertsQuery = useQuery({
    queryKey: ['dashboard', 'debt-alerts'],
    queryFn: () => getDebtAlerts({}),
  });
  ...
  const paymentData = useMemo(
    () =>
      Object.entries(paymentsQuery.data ?? {}).map(([method, total]) => ({
  ...
  const topProductsData = (topProductsQuery.data ?? []).map((item) => ({
    name:
    ...
    <Stack spacing={3}>
      ...
      >
        <Box>
          ...
          <Typography color="text.secondary">
            Hola, {user?.name || 'usuario'}. Vista operativa para {roleLabel}.
          </Typography>
        </Box>
        ...
        {kpiQuery.isLoading ? (
          ...
          <>
            ...
            <KpiCard
              ...
              subtitle={`Periodo de pagos: ${periodDays} días`}
            ...
            <KpiCard
              ...
              value={formatQuantity(central?.availableStock ?? 0)}
      ...
      <Box
        sx={{
          ...
          gridTemplateColumns: {
            xs: '1fr',
            lg: canSeeSales ? 'minmax(0, 2fr) minmax(320px, 1fr)' : '1fr',
          },
          gap: 2,
        }}
      >
              ...
              <ResponsiveContainer>
                <LineChart data={trendData}>
                  ...
                  <ChartTooltip
                    ...
                    labelFormatter={(value) => formatDate(`${value}T12:00:00`)}
                  />
                  <Line
                    type="monotone"
                    dataKey="total"
      ...
      >
        <Section
          ...
          subtitle="Existencia, reserva y disponibilidad actual"
          action={
            <Button
        ...
        >
          <Stack spacing={2.5}>
            {(kpi?.stockByWarehouse ?? []).map((warehouse) => {
              const availablePercentage =
                warehouse.totalStock > 0
                  ? Math.max(
                      0,
                      (warehouse.availableStock / warehouse.totalStock) * 100,
                    )
                  : 0;
              return (
                <Box key={warehouse.id}>
                  ...
                  >
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {warehouse.productsCount} productos ·{' '}
                        {formatQuantity(warehouse.reservedStock)} reservados
                      </Typography>
                    </Box>
                    <Typography sx={{ fontWeight: 700 }}>
                  ...
                  <LinearProgress
                    ...
                    color={availablePercentage < 25 ? 'warning' : 'success'}
              ...
              <TableBody>
                {(lowStockQuery.data ?? []).slice(0, 6).map((item) => (
                  <TableRow key={item.productId} hover>
                    ...
                    <TableCell align="right">
                      <Typography
                        color={item.availableStock <= 0 ? 'error' : 'warning.main'}
                        sx={{ fontWeight: 700 }}
                      >
